...
const detectSections = (input) => {
    ...
    const pushCurrentSection = () => {
        const hasContent = Boolean(currentSection.heading) ||
    ...
    for (const block of blocks) {
        ...
        const bulletLines = lines.filter((line) => bulletRegex.test(line) || numberedBulletRegex.test(line));
        if (bulletLines.length === lines.length) {
            currentSection.bullets = [
                ...(currentSection.bullets ?? []),
                ...bulletLines.map((line) => line.replace(bulletRegex, "").replace(numberedBulletRegex, "").trim()),
            ];
            continue;
        }
